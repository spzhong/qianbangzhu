...
public class Data {
	...
	public void addDataToList() {
		...
		if (this.urlNum == "1") {// 登录
			...
			API_USRXXGLLIST_29();
		} else if (this.urlNum == "371") {// 我的通知消息
			API_USRXXGLLIST_371();
	...
	private void API_LOGIN_1_2_3() {
		...
		String bendi = Tool.readData((Context) this.getActivity(), "user",
				"yhzh");
		// 判断是否相等网络 本地
	...
	// 添加数据到数组中--我的通知消息 -平台公告
		private void API_USRXXGLLIST_371() {
			JSONObject result = (JSONObject) this.resultObj;
			JSONArray jsonArray = null;
			JSONObject json = null;
			try {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					try {
						json = jsonArray.getJSONObject(i);
						Notification notification = new Notification();
						notification.initMakeData_listInfo2(json);
						this.list.add(notification);// 加入list中
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
